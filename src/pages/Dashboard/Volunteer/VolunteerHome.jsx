...
export const VolunteerHome = () => {
    ...
    const { data: totalUsers = 0 } = useQuery({
        queryKey: ['totalUsers'],
        queryFn: async () => {
            const res = await axiosSecure.get('/users/counts');
    ...
    const { data: totalRequests = 0 } = useQuery({
        ...
        queryFn: async () => {
            const res = await axiosSecure.get('/requestdonations/counts');
            return res.data.count;
        }
    });
    ...
    return (
        <div className="container mx-auto my-10 p-6 bg-white rounded-lg shadow-md">
            <div className="text-center mb-10">
                <h1 className="text-4xl font-bold">Welcome, {user?.displayName || 'Volunteer'}!</h1>
